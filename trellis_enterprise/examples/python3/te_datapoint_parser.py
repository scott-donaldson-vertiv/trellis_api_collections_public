# ---------------------------------------------------------------------------------------------
# Title:		    Trellis(tm) Enterprise - Datapoint Parser
# Description:      Returns collected data by one or more datapoint programmatic names.
# Script Name:      te_datapoint_parser.py
# Created:          2019/11/01
# Modified:         2021/06/03
...
PATH_BUILDINGS_SEARCH_FBQ = 'devices/search/findByQuery'
...
class HttpClient:
    ...
    def _request_send_limited(self, url: str, query_params: (dict, None)) -> HTTPResponse:
        ...
        print("request ended, status={}, elapsed_seconds={}".format(response.status_code, response.elapsed.total_seconds()))
...
def get_sensors(http_conn: HttpClient, match_criteria: dict) -> (json, None):
    """
    Fetch sensors that match owner and device name criteria.

    Changes - Modified search to include name contains in the search criteria, this should further reduce results given
    the test criteria will explicitly match on name.

    :param http_conn: urllib3 connection.
    :param match_criteria: User defined search criteria.
    :return:
    """
    page = 0
    devices: dict = []
    ...
    while True:
        sensor_params = {
            "programmaticName": "SENSOR",
            "name": match_criteria,
            "projection": "summary",
            "page": page,
            "size": SENSORS_PAGE_SIZE,
        }
        result = http_conn.request_send(PATH_DEVICES_SEARCH_FBNICAOCPNIC, sensor_params)

        page += 1

        i: str = 0
        for device in result['_embedded']['devices']:
            if not test.is_not_test_sensor(device['name']):
                devices.update({
                    i : {
                        "id": device['id'],
                        "name": device['name']}
...
def get_devices_under_building(http_conn: HttpClient, building_id: (uuid, str), match_criteria: dict) -> \
        (json, None):
    """
    Returns sensors by building identifier.

    Changes - Modified to include name contains within search to further reduce results that need to be processed as these
    are part of the test criteria anyway.

    :param http_conn: urllib3 session
    :param building_id: UUID of building
    :param owner:
    ...
    page = 0
    devices: json = []
    test = TestCriteria(owner=match_criteria['owner'], match=match_criteria['name'])
    device_list: dict = {}

    # TODO: The builidingId should be sanity checked, preferable input type would be UUID to ensure correctness.

    # Verify total results before proceeding.
    query_sensor_criteria = \
        '(displayCategoryProgrammaticName==SENSOR and buildingId==' + building_id + ' and name==*'\
        + match_criteria['name'] + '*) '

    params = {
        "query": query_sensor_criteria,
        "page": 0,
        "size": SENSORS_PAGE_SIZE,
    }

    buildings = http_conn.request_send(PATH_BUILDINGS_SEARCH_FBQ, params)

    if DEFAULT_DEBUG_VERBOSITY > 1:
        logger.debug("Query response:")
        logger.debug(json.dumps(buildings))

    if buildings['page']['totalElements'] > 0:

        while True:
            query_sensor_criteria = "(displayCategoryProgrammaticName==SENSOR and buildingId==" + building_id + " and name==*" + match_criteria['name'] + "*)"
            params = {
                "query": query_sensor_criteria,
                "page": page,
                "size": SENSORS_PAGE_SIZE,
            }
            result = http_conn.request_send(PATH_BUILDINGS_SEARCH_FBQ, params)
            page += 1
            ...
            try:
                ...
                logger.exception("Payload processing failure.")
                logger.exception(ex)
                return None
...
def get_sensor_location(http_conn: HttpClient, sensor_id: (uuid, str)) -> (json, None):
    """
    Returns the location string for sensor.

    Changes - Projection type from 'all' with 'search' as payload is ~1/6th the size but still returns 'location'
    node.

    :param http_conn: The urllib3 session.
    :param sensor_id: The uuid of the target sensor.
    :return: The json payload or None if invalid.
    """

    # TODO: Sensor ID should be validated.
    ...
    try:
        defs = http_conn.request_send(sensor_id, detail_params)
    ...
    try:
        ...
        logger.exception("Response processing failure.")
        logger.exception(str(ex))
...
def get_datapoint_definitions(http_conn: HttpClient, sensor_id: (str, uuid)) -> (json, None):
    """
    Returns definitions for data points.
    ...
    # TODO: Sanity check the provided sensor_id is valid UUID.

    try:
        defs = http_conn.request_send(PATH_DATA_POINT_DEFS, div_params)

        logger.debug(defs)
        if DEFAULT_DEBUG_VERBOSITY > 1:
            logger.debug("Response:")
            logger.debug(str(json.dumps(defs)))
    except HTTPError as ex:
        logger.exception("Connection failure.")
        logger.exception(str(ex))

    try:
        def_tmp = defs['_embedded']['datapointdefinitions']
        def_name = [ def_tag['tag'] for def_tag in def_tmp ]
        logger.debug("Data-point definitions:")
        logger.debug(str(def_name))
        # []['tag']
    except KeyError as ex:
        logger.exception("Response processing failure.")
        logger.exception(str(ex))
...
def print_data_points(http_conn: HttpClient, def_name, row, match_criteria: dict):
    # set parameters
    time_now = datetime.utcnow()
    time_from = (time_now - timedelta(days=int(match_criteria['days']))).strftime(DEFAULT_TIMESERIES_FORMAT)
    # time_from = (time_now - timedelta(days=3)).strftime(DEFAULT_TIMESERIES_FORMAT)
    # time_to = (time_now - timedelta(days=2)).strftime(DEFAULT_TIMESERIES_FORMAT)
    time_to = time_now.strftime(DEFAULT_TIMESERIES_FORMAT)
    time_params = {
        'from': time_from,
        'to': time_to,
        'ownerId': row["id"],
        'dpProgrammaticName': def_name,
        'size': METRICS_PAGE_SIZE,
    }

    result = http_conn.request_send(PATH_DATA_POINTS, time_params)

    try:
        # vtime = result['_embedded']['datapoints'][0]['timestamp']
        values = result['_embedded']['datapoints']
        # print ("%s,%s,%s,%s" %(row["name"],response.status_code,vtime,value))
        print("%s,%s" % (row["name"], values))
        # print "name={},data_count={}".format(row["name"],len(values))

    except KeyError as ex:
        logger.exception("Response processing failure.")
        logger.exception(str(ex))
        print("name={},key error,body={}".format(row["name"], result))
...
def main(arguments):
    ...
    if 0 < arguments.match_days <= 365:
        # TODO: Validate string is only Trellis accepted chars.
        _criteria['days'] = arguments.match_days

    if arguments.datapoint_tags is not None:
        def_proc = arguments.datapoint_tags
        logger.debug(str(arguments.datapoint_tags))
    else:
        def_proc: list = []
    ...
    print("==== get all buildings start! ====")
    logger.info("==== get all buildings start! ====")
    buildings: json = get_buildings(http_conn, _criteria)
    print("==== get all buildings end! count={} ====".format(len(buildings)))
    logger.info(("==== get all buildings end! count={} ====".format(len(buildings))))

    # get all sensor
    # print "===== get all sensor start! ====="
    # logger.info("===== get all sensor start! =====")
    # sensors = get_sensors(session)
    # print "===== get all sensor end! count={} =====".format(len(sensors))
    # logger.info(("===== get all sensor end! count={} =====".format(len(sensors))))

    # get sensors under the building
    sensors: int = 0
    i: int = 0
    j: int = 0
    for name, building in buildings.items():
        i+=1
        j+=1
        logger.debug(str(building))
        logger.debug(("Output sensors in building: " + building['name']))
        print("get sensors under the building start! name={} ====".format(building['name']))
        sensors = get_devices_under_building(http_conn, building["id"], _criteria)
        print("===== get sensors under the building end! count={} =====".format(len(sensors)))

        for name, sensor in sensors.items():
            print("===== get sensor info start! name={}====".format(sensor["name"]))
            location = get_sensor_location(http_conn, sensor["id"])
            print("===== get sensor info end! name={} location={}====".format(sensor["name"], location))

            print("===== get def_name start! name={}=====".format(sensor["name"]))
            def_name = get_datapoint_definitions(http_conn, sensor["id"])
            if def_name is not None:
                for definition in def_name:
                    print("===== get def_name end! name={} (id={}) ProgName={}=====".format(sensor["name"],sensor["id"], definition))
                    print("===== get measure values start! name={}=====".format(sensor["name"]))
                    if ((not def_proc) or (definition in def_proc)):
                        print_data_points(http_conn, definition, sensor, _criteria)
                    else:
                        logger.debug("Datapoint " + definition + " not in selected output.")
                    print("===== get def_namemeasure values end! name={}=====".format(sensor["name"]))
            logger.info("Processed sensor: " + str(j) + " of " + str(len(sensors)))
        logger.info("Processed building: " + str(i) + " of " + str(len(buildings)))
    ...
    if len(sensors) > 0:
        sensor_per_min: float = ((len(sensors)*60) / elapsed_real_time)
    else:
        sensor_per_min: float = 0.0

    logger.info(("Duration: process_time={}s real_time={}s sensors_per_min={}/min".format(elapsed_process_time, elapsed_real_time, sensor_per_min)))
...
if __name__ == '__main__':
    ...
    arg_group_criteria.add_argument('-d', '--match-days', help='The number of previoys days to return.', type=int,
